...
# Train the model
model = object_detector.create(train, model_spec=spec, batch_size=4, validation_data = val, epochs = 200, train_whole_model=True)
...
import pickle
with open("losses_long.pkl", "wb") as fp:   #Pickling
    pickle.dump(loss, fp)
with open("val_losses_long.pkl", "wb") as fp:   #Pickling
...
# 4. Model Compilation
model.export(export_dir='/content/drive/MyDrive/ColabNotebooks/Zira/', tflite_filename='boards_long.tflite')

# Evaluate the tflite model
tflite_eval_result = model.evaluate_tflite('/content/drive/MyDrive/ColabNotebooks/Zira/boards_long.tflite', val)

# Print COCO metrics for validation data
print("COCO metrics Original Model Longer Training")
for label, metric_value in tflite_eval_result.items():
    print(f"{label}: {metric_value}")

# 5. Apply Model Quantization
config = QuantizationConfig.for_float16()
model.export(export_dir='/content/drive/MyDrive/ColabNotebooks/Zira/', tflite_filename='quant_long.tflite', quantization_config=config)

# Evaluate the quant.tflite model
quant_tflite_eval_result = model.evaluate_tflite('/content/drive/MyDrive/ColabNotebooks/Zira/quant_long.tflite', val)

# Print COCO metrics for quantized model 
print("COCO metrics quant_long.tflite")
